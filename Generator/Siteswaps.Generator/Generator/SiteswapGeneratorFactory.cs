...
public record SiteswapGeneratorFactory
{
    ...
    public SiteswapGeneratorFactory ConfigureFilter(Func<IFilterBuilder, IFilterBuilder>? builder) => this with
    {
        Config = builder is null ? Config : Config.Add(builder)
    };
    
    public SiteswapGenerator Create(SiteswapGeneratorInput input) => 
        new(Config.Aggregate(new FilterBuilder().WithInput(input), (current, func) => func(current)).WithDefault().Build(), 
            input);
}