...
public record FilterBuilder(SiteswapGeneratorInput Input) : IFilterBuilder
{
    ...
    public IFilterBuilder Not(ISiteswapFilter filter) => this with { Filter = Filter.Add(new Combinatorics.NotFilter(filter)) };
    ...
    public IFilterBuilder Or(params IEnumerable<ISiteswapFilter> filter) => this with { Filter = [new OrFilter(filter)] };
    public IFilterBuilder WithState(State state) => this with { Filter = Filter.Add(new StateFilter(Input, state)) };
    public IFilterBuilder FlexiblePattern(List<List<int>> pattern, int numberOfJuggler, bool isGlobalPattern) => this with { Filter = Filter.Add(new FlexiblePatternFilter(pattern, numberOfJuggler, Input, isGlobalPattern)) };
    ...
    public IFilterBuilder Pattern(IEnumerable<int> pattern, int numberOfJuggler)
    {
        ...
        return this with
        {
            Filter = Filter.Add(PatternFilterHeuristicBuilder.Build(pattern)).Add(new FlexiblePatternFilter(pattern.Select(x => new List<int>() { x }).ToList(), numberOfJuggler, Input, true)),
        };
    }
}
