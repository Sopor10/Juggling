...
public class SiteswapPrompts
{
            ...
            Content = new TextContentBlock
            {
                Text = $"""
                    ...
                    1. Use the 'generate_siteswaps' tool with appropriate parameters:
                       ...
                       - maxHeight: based on level (beginner: {numberOfObjects
                        + 2}, intermediate: {numberOfObjects + 4}, advanced: {numberOfObjects + 6})
    ...
    public static IEnumerable<PromptMessage> FindTransitions(
        [Description("Source siteswap (e.g., '3')")] string fromSiteswap,
        [Description("Target siteswap (e.g., '531')")] string toSiteswap,
        [Description("Maximum transition length (default: 3)")] int maxLength = 3
    )
    {
        yield return new PromptMessage
        {
    ...
    public static IEnumerable<PromptMessage> LearnConcept(
        [Description(
            "The concept to explain (e.g., 'orbit', 'state', 'transition', 'hijacking', 'multiplex', 'sync')"
        )]
            string concept
    )
    {
        var resourceSuggestion = concept.ToLower() switch
        {
            ...
            "state" or "states" or "ground state" or "excited state" =>
                "siteswap:definition:juggling-state or siteswap:definition:ground-state",
            "transition" or "transitions" => "siteswap:definition:transition-throw",
            "hijacking" or "hijack" =>
                "siteswap:definition:hijacking and get_passing_zone_hijacking",
    ...
    public static IEnumerable<PromptMessage> CreatePracticePlan(
        [Description("The target siteswap to learn")] string targetSiteswap,
        [Description(
            "Current skill: Which patterns can the user already do? (comma-separated, e.g., '3,423,441')"
        )]
