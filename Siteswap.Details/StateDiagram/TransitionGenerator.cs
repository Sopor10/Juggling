...
public static class TransitionGenerator
{
    ...
    private static IEnumerable<ImmutableList<int>> Recurse(
        State fromState,
        State toState,
        ImmutableList<int> transitionSoFar,
        int length,
        int maxHeight
    )
    {
        foreach (var transition in fromState.Transitions(maxHeight))
        {
            ...
            foreach (
                var immutableList in Recurse(
                    transition.N2,
                    toState,
                    transitionSoFar.Add(transition.Data),
                    length - 1,
                    maxHeight
                )
            )
    ...
    {
        return StateGraphFromSiteswapGenerator.CalculateGraph(
            siteswap.Items.EnumerateValues(1).ToArray(),
            length
        );
    }
}
